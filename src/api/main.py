...
load_dotenv()  # load DEMO_MODE and other vars before agent imports
...
# Maps shipment_id → current thread_id.
# A new thread_id is generated each time init is called, giving a completely
# fresh in-memory checkpoint (operator.add fields start at [], not accumulated).
...
def _config(shipment_id: str) -> dict:
    """Return LangGraph config using the current thread_id for this shipment."""
    thread_id = _thread_map.get(shipment_id, shipment_id)
    return {"configurable": {"thread_id": thread_id}}


def _obj_to_dict(obj) -> dict | None:
    """Convert Pydantic model or plain dict → dict safely."""
    if obj is None:
        return None
...
async def init_shipment(shipment_id: str):
    # Generate a brand-new thread_id → guaranteed empty MemorySaver checkpoint.
    # This is cleaner than trying to purge operator.add fields from an existing thread.
...
async def run_scenario(scenario_name: str, shipment_id: str):
    ...
    # Init creates a fresh thread_id — guaranteed clean slate
